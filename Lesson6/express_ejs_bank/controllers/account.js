...
router.get('/transactions', async (req, res) => {
    if(res.locals.user.accounts && res.locals.user.accounts.length > 0){
        try {
            ...
            res.renderWithLayout('account/transactions', {transactions: null, error: error.message});
        }
    } else {
        res.renderWithLayout('account/transactions', {transactions: null, error: 'No account found for user'});
    }   
});
